    ...
    def reproduction(self):
        ...
        if self.resources // self.cost >= 0:
            for young in range(np.random.poisson(self.resources // self.cost)):
                ...
                meta.list_x.append(self.x)
        # after the individual has reproduced it will die
        self.die()
...
class Metapopulation:
    ...
    def __init__(self, max_x, max_y):
        ...
        self.max_x = max_x
        self.max_y = max_y
        p = suitable_habitat # proportion of suitable habitat
        n0 = 1 - p
    ...
    def a_day_in_the_life(self):
        '''An hour in the life of the individuals'''
        ...
        for patch in self.list_resources:
            self.list_regrowth.append((patch[2] * regrowth) * (1 - self.environment[patch[0]][patch[1]] / (patch[2] * carrying_capacity)))
            self.environment[patch[0]][patch[1]] += (patch[2] * regrowth) * (1 - self.environment[patch[0]][patch[1]] / (patch[2] * carrying_capacity))
...
with open("random walk with heterogeneity.csv", "w") as csvfile:
    writer = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
    writer.writerow(['mass (kg)', 'autocorrelation', 'suitable habitat', 'run', 'step mean',
                     'mean divergence', 'population size', 'mean resources', 'mean regrowth', 'mean x'])

with open(f'runs random walk with heterogeneity (p = {list_p[-1]}).csv', "w") as csvfile:
    writer = csv.writer(csvfile, delimiter=',', quotechar='|', quoting=csv.QUOTE_MINIMAL)
...
for autocorrelation in list_h:
    for suitable_habitat in list_p:
        ...
        ax5.set_title('mean resource regrowth')
        ...
        for simulation in range(runs):
            ...
            for timer in range(generations):
                if timer == 0:
                    for patch in meta.list_resources:
                        r = rnd.uniform(0, 2)
                        patch.append(r)
                ...
                if timer == generations - 1:
                    ...
                    for i in sample_pop:
                        ...
                        with open(f'runs random walk with heterogeneity (p = {list_p[-1]}).csv', 'a', newline='') as ind:
            ...
            with open("random walk with heterogeneity.csv", 'a', newline='') as f:
                    fwriter = csv.writer(f)
                    fwriter.writerow([f'{mass}', f'{autocorrelation}', f'{suitable_habitat}',
        ...
        fig.savefig(f'random walk with heterogeneity (mass = 1g, p = {suitable_habitat} and h = {autocorrelation}).png', dpi=200)
        ...
        result.save(f'allele distribution random walk with heterogeneity (mass = 1g, p = {suitable_habitat} and h = {autocorrelation}).png', dpi=(300, 300))
